...
import kotlinx.android.synthetic.main.fragment_current_employee.*
import ru.meteor.myapplication.R
import ru.meteor.myapplication.adapters.Utilities
import ru.meteor.myapplication.models.Employee
import ru.meteor.myapplication.models.Speciality
import java.util.*

class CurrentEmployeeFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_current_employee, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (bundle != null) {
            employee = bundle.getParcelable(employeeKey)
        }

        // Если не нулевой у нас сотрудник, то пишем все в поля
        if (employee != null) {
            ...
            employeeFirstName.text =
                Utilities.firstUpperCase(employee.fName)
            // Фамилия
            employeeSecondName.text =
                Utilities.firstUpperCase(employee.lName)
            // Возраст

            if (!employee.birthday.isNullOrEmpty()) {
                val year: Int = Utilities.parseDate(employee.birthday!!).get(Calendar.YEAR)
                val month: Int = Utilities.parseDate(employee.birthday!!)
                    .get(Calendar.MONTH)
                ...
                if (Utilities.checkDateFormat(
                        Utilities.replaceDate(
                            employee.birthday!!
                        )
                    )
                ) {
